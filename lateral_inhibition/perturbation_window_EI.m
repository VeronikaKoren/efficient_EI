...
for ii=1:length(d_stim_vec)

    disp(n-ii)
    
    d_stim=d_stim_vec(ii);               % duratio of stimulation
...
if saveres==1
    ...
    savename='perturbation_Ei_spont_dstim';
    save([savefile,savename],'minfE','minfI','semfE','semfI','mtar','semtar','namepop','ntr','d_stim_vec');
end
...
if showfig==1
    ...
    figure('name','window_length','Position',[0,0,12,12]);

    subplot(3,1,1)
    hold on
    plot(d_stim_vec,mtar-semtar,'k')
    plot(d_stim_vec,mtar+semtar,'k')

    subplot(3,1,2)
    hold on
    for k=1:2
        plot(d_stim_vec,minfI{k}-semfI{k},'color',col{k})
        plot(d_stim_vec,minfI{k}+semfI{k},'color',col{k})
    end
    hold off
    ...
    hold on
    for k=1:2
        plot(d_stim_vec,minfE{k}-semfE{k},'color',col{k})
        plot(d_stim_vec,minfE{k}+semfE{k},'color',col{k})
    end
    hold off
    xlabel('length of stimulation window')
end
