function [I_E,I_I,r,mse,kappa,CV,fr] = current_fun_taus(dt,sigmav,beta,tau_vec,s,N,q,d,x)
...
FF=mean(ff_time);   % average across neurons
...
[~,kappa] = performance_fun(x,xhat_e,xhat_i,re,ri);
mse_e=mean((x-xhat_e).^2,2); % mean across time (per dt)
mse_i=mean((xhat_e-xhat_i).^2,2);

mse=cat(2,mse_e,mse_i);
