...
[rmse] = performance_fun(x,xhat_e,xhat_i);

%% Coefficient of variation

yer=int8(fe); % integer format
yir=int8(fi);

y1e=sum(reshape(yer,N,T*dt,1/dt),3); % bin the spike trains in 1ms bins
y1i=sum(reshape(yir,Ni,T*dt,1/dt),3);
